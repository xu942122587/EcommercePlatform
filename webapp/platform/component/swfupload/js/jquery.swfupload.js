(function($) {
	...
	$.hz.swfupload = {
		...
		validateAttachment:function(objId,isMustUploadFile){//验证接口
			var $obj = $("#"+objId);
			var result = true;
			$obj.each(function(){
				if(null == isMustUploadFile){
					isMustUploadFile = true;
				}
				if(isMustUploadFile){
				...
				for(var j = 0;j < vals.values().length; j++){
					...
					if(uf.status != UPLOADFILE_STATUS_UPLOADED){
						if(uf.proc == 100){
							toastr.error($.i18n.prop('upload_js_processing'));
						}else{
							toastr.error($.i18n.prop('upload_js_uploading'));
						}
						result = false;
						break;
					}else{
	...
	function loadUploadFlash(param){
		...
		if(param.existAttachments && param.existAttachments.length>0){
			...
			$(param.existAttachments).each(function(){
				...
				htmlLi += "	<img src='" + com.ue.global.path + "/sys/attachment/showPic.do?downloadToken=" + uf.id + "6210e537ab7425ada8f8cd2430ccb36f' width='100px' height='100px' alt='' /><br />";
...
//				button_window_mode : "opaque",//防止flash在最上层
				button_window_mode : SWFUpload.WINDOW_MODE.TRANSPARENT,//防止flash在最上层
	...
	function addNewFile(file){
		...
		html += "<br/>	<span id='" + divId + "Pb" + uf.id + "'></span>&nbsp";
	...
	function checkRequired(){
		...
		if(requiredFlag){
			...
			if(getExistFileCount.apply(this) == 0){
				//没有附件
				canCommit = false;
				toastr.error($.i18n.prop('upload_js_required'));
			}
			//else{
			//已有附件
	...
	function queueError(file,errorCode,message){
		...
		if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
			toastr.error($.i18n.prop('upload_js_fileCountAllow',param.maxFileCount));
			return;
		}
		if (errorCode === SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT){
			toastr.error($.i18n.prop('upload_js_fileSizeNotAllow',param.maxFileSize));
			return;
		}
		if (errorCode === SWFUpload.QUEUE_ERROR.INVALID_FILETYPE){
			toastr.error($.i18n.prop('upload_js_notAllowFileType',param.limitFileExtention));
			return;
		}
	}
