...
<script type="text/javascript">
	...
	Namespace.register("com.yunt.consumeBonusDetail.list");
	com.yunt.consumeBonusDetail.list = {
		refresh : function(isSelect) {
			var href_string = '${path}/consumeBonusDetail/list?model.guideUser.username=' + $("#model_guideUser_username").val().trim();
			if(!isSelect){
			...
			<div class="col-xs-6">
				<form class="form-inline pull-right" action="" id="search" >
					...
					<button type="button" class="btn btn-default"
						onclick="com.yunt.consumeBonusDetail.list.refresh(true)">筛选</button>
				</form>
			</div>
		</div>

	</div>
	<form id="theForm" action="${path}/consumeBonusDetail/list?model.guideUser.username=${model.guideUser.username }">
			...
			<table class="table table-striped table-hover table-bordered text-center"
				...
				<thead class="text-center">
					<tr>
						...
						<th class="text-center">推荐充值总金额</th>
						<th class="text-center">充值提成总金额</th>
						<th class="text-center">操作</th>
					</tr>
				</thead>
				<tbody>
					<c:forEach items="${pageObj.results}" var="consumeBonusDetail" varStatus="i">
						<!-- 系统管理员 -->
						<tr>
							<td class="text-center">${(i.index+1)+((pageObj.pageNo-1) * pageObj.pageSize)}</td>
							<td class="text-center">${consumeBonusDetail.guideUser.username }</td>
							<td class="text-center">${consumeBonusDetail.guideUser.name }</td>
							<td class="text-center">${consumeBonusDetail.guideUser.phone }</td>
							<td class="text-center">${consumeBonusDetail.sumTargetMoney }</td>
							<td class="text-center">${consumeBonusDetail.sumMoney }</td>
							<td class="text-center">
								<c:if test="${fn:indexOf(user.role.operationCodes,'consumeBonusDetail_view') > -1 }">
									<input type="button" value="查看" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#edit-device"
											onclick="com.yunt.index.loadInner('${path}/consumeBonusDetail/view?guideUserId=${consumeBonusDetail.guideUser.id }','查看充值提成详情')" />
									&nbsp;&nbsp; 
								</c:if>
							</td>
						</tr>
