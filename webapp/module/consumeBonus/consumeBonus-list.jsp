...
<script type="text/javascript">
	...
	Namespace.register("com.yunt.consumeBonus.list");
	com.yunt.consumeBonus.list = {
		refresh : function(isSelect) {
			var href_string = '${path}/consumeBonus/list';
			if(!isSelect){
		...
		doDelete : function(id){
			...
			$("#deleteButton").unbind("click").click(function(){
				$.ajax({
					url : "${path}/consumeBonus/doDelete?id="+id,
					type : "POST",
					dataType : "json",
					success : function(data) {
						$('#deleteModal').modal('hide');
						 if (data.success) {
							...
							com.yunt.consumeBonus.list.refresh();
						} else {
...
<body>
	...
	<div class="toolbar">
		<div class="row">
			<div class="col-xs-6">
				<c:if test="${fn:indexOf(user.role.operationCodes,'consumeBonus_input') > -1 }">
					<div class="quick-add-btn">
						<a href="javascript:void(0)"type="button" class="btn btn-danger"
								onclick="com.yunt.index.loadInner('${path}/consumeBonus/input','添加提成')">
	...
	<form id="theForm" action="${path}/consumeBonus/list">
		...
		<div class="panel panel-default" id="all-device-detail">
			...
			<table class="table table-striped table-hover table-bordered text-center"
				...
				<tbody>
					<c:forEach items="${pageObj.results}" var="consumeBonus" varStatus="i">
						<!-- 系统管理员 -->
						<tr>
							<td class="text-center">${(i.index+1)+((pageObj.pageNo-1) * pageObj.pageSize)}</td>
							<td class="text-center">${consumeBonus.startMoney}</td>
							<td class="text-center">% ${consumeBonus.proportion}</td>
							<td class="text-center">
								<c:if test="${fn:indexOf(user.role.operationCodes,'consumeBonus_input') > -1 }">
									<input type="button" value="修改" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#edit-device"
											onclick="com.yunt.index.loadInner('${path}/consumeBonus/input?id=${consumeBonus.id }','修改返现信息')" />
									&nbsp;&nbsp; 
								</c:if>
								<c:if test="${fn:indexOf(user.role.operationCodes,'consumeBonus_delete') > -1 }">
									<a class="btn btn-default btn-xs" href="javascript:void(0);" onclick="com.yunt.consumeBonus.list.doDelete('${consumeBonus.id }')">删除</a>
									&nbsp;&nbsp; 
								</c:if>
							</td>
						</tr>
