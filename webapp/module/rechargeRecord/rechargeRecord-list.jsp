	...
	com.yunt.rechargeRecord.list = {
		refresh : function(isSelect) {
			...
			if(!isSelect){
				...
				if($("#rechargeRecord_endDate_select").val().trim() != ''){
					if(compareCurrentDate($("#rechargeRecord_endDate_select").val())){
						toastr.warning('结束日期应当小于或等于当前日期！');
						return;
					}
				}
				if($("#rechargeRecord_startDate_select").val().trim() != ''){
					if(compareCurrentDate($("#rechargeRecord_startDate_select").val())){
						toastr.warning('开始日期应当小于或等于当前日期！');
						return;
					}
				}
				if($("#rechargeRecord_startDate_select").val().trim() != '' && $("#rechargeRecord_endDate_select").val().trim() != ''){
					if(!compareDate($("#rechargeRecord_endDate_select").val(),$("#rechargeRecord_startDate_select").val())){
			...
			<div class="col-xs-6">
				<form class="form-inline pull-right" action="" id="search" >
					<div class="form-group">
						<input type="text" value="${model.customer.phone }" id="phone" name="phone" class="form-control" placeholder="用户电话" maxlength="20">
					...
					<button type="button" class="btn btn-default"
						onclick="com.yunt.rechargeRecord.list.refresh(true)">筛选</button>
				</form>
			</div>
		</div>
				...
				<tbody>
					<c:forEach items="${pageObj.results}" var="rechargeRecord" varStatus="i">
						...
						<tr>
							...
							<td class="text-center">${rechargeRecord.guideUser.username }</td>
							<td class="text-center">${rechargeRecord.money }</td>
							<td class="text-center">${rechargeRecord.backCash }</td>
							<td class="text-center"><c:if test="${'1' eq rechargeRecord.state  }">完成</c:if></td>
