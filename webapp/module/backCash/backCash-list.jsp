...
<script type="text/javascript">
	...
	Namespace.register("com.yunt.backCash.list");
	com.yunt.backCash.list = {
		refresh : function(isSelect) {
			var href_string = "${path}/backCash/list";
			if(!isSelect){
		...
		doDelete : function(id){
			...
			$("#deleteButton").unbind("click").click(function(){
				$.ajax({
					url : "${path}/backCash/doDelete?id="+id,
					type : "POST",
					dataType : "json",
					success : function(data) {
						$('#deleteModal').modal('hide');
						 if (data.success) {
							...
							com.yunt.backCash.list.refresh();
						} else {
...
<body>
	...
	<div class="toolbar">
		<div class="row">
			<div class="col-xs-6">
				<c:if test="${fn:indexOf(user.role.operationCodes,'backCash_input') > -1 }">
					<div class="quick-add-btn">
						<a href="javascript:void(0)"type="button" class="btn btn-danger"
								onclick="com.yunt.index.loadInner('${path}/backCash/input','添加返现')">
							<span class="glyphicon glyphicon-new-window"></span> 添加返现
						</a>
					</div>
				</c:if>
			</div>
	...
	<form id="theForm" action="${path}/backCash/list">
		...
		<div class="panel panel-default" id="all-device-detail">
			...
			<table class="table table-striped table-hover table-bordered text-center"
				...
				<tbody>
					<c:forEach items="${pageObj.results}" var="backCash" varStatus="i">
						<!-- 系统管理员 -->
						<tr>
							<td class="text-center">${(i.index+1)+((pageObj.pageNo-1) * pageObj.pageSize)}</td>
							<td class="text-center">${backCash.startMoney}</td>
							<td class="text-center">% ${backCash.proportion}</td>
							<td class="text-center">
								<c:if test="${fn:indexOf(user.role.operationCodes,'backCash_input') > -1 }">
									<input type="button" value="修改" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#edit-device"
											onclick="com.yunt.index.loadInner('${path}/backCash/input?id=${backCash.id }','修改返现信息')" />
									&nbsp;&nbsp; 
								</c:if>
								<c:if test="${fn:indexOf(user.role.operationCodes,'backCash_delete') > -1 }">
									<a class="btn btn-default btn-xs" href="javascript:void(0);" onclick="com.yunt.backCash.list.doDelete('${backCash.id }')">删除</a>
									&nbsp;&nbsp; 
								</c:if>
							</td>
						</tr>
