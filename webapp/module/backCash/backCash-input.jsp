...
<script>
	var isSuccess = true;
	Namespace.register("com.yunt.backCash.input");
	com.yunt.backCash.input = {
		submitbackCash : function() {
			if ($('#theForm').data('bootstrapValidator').isValid()) {
				...
				if (isSuccess) {
					$.ajax({
						url : "${path}/backCash/saveOrUpdate",
						data : formData,
						type : "POST",
						async : false,
						dataType : "json",
...
<body>
	...
	<div class="row">
		<div class="col-xs-12">
			<div class="display">
				<form onsubmit="return com.yunt.backCash.input.submitbackCash()"
					...
					<div class="form-group">
						<label for="product-uuid"
							class="col-xs-12 control-label lantiao_lable">返现信息</label>
					</div>

					<div class="form-group">
						<label for="product-uuid" class="col-xs-2 control-label">返现档位</label>
						<div class="col-xs-8">
							<input type="text" value="${model.startMoney }" class="form-control"
								onkeydown="if(event.keyCode==32) return false;"
								name="model.startMoney" required data-bv-notempty-message="返现档位不能为空"
								pattern="^(([1-9][0-9]{0,7})(\.{1}[0-9]{1,2}){0,1}|([0-9]{1}\.{1}[0-9]{0,1}[1-9])|0|0.0|0.00)$"
								data-bv-regexp-message="请输入正确的返现档位，eg:18.88"/>
						</div>
					</div>

					<div class="form-group">
						<label for="product-uuid" class="col-xs-2 control-label">返现比例</label>
						<div class="col-xs-8">
							<input type="text" value="${model.proportion }" class="form-control"
								onkeydown="if(event.keyCode==32) return false;"
								name="model.proportion" required data-bv-notempty-message="返现比例不能为空"
								pattern="^(([1-9][0-9]{0,1})(\.{1}[0-9]{1,2}){0,1}|([0-9]{1}\.{1}[0-9]{0,1}[1-9])|0|0.0|0.00|100)$"
								data-bv-regexp-message="请输入正确的返现比例，eg:18.88"/>
						</div>
					</div>
