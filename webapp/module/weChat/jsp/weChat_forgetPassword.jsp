...
<html lang="zh-CN">
	...
	<body class="bg_login">
		...
		<form id="theForm">
			...
			<input class="login_input" type="password" name="newPassword" id="newPassword_input" placeholder="请输入新密码"></input>
		</form>
		
		<!--保存按钮 -->
		<button type="button" class="login_btn" onclick="doForgetPassword()">保存</button>
		...
		function doForgetPassword() {
			...
			if(!phoneReg.test(phone)){
				...
				if (isSuccess) {
					showLodding();
					$.ajax({
						url : "${path}/weChat/doForgetPassword",
						data : formData,
						type : "POST",
						dataType : "json",
						success : function(data) {
