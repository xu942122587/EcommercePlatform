...
<script type="text/javascript">
	...
	Namespace.register("com.yunt.registerBonusDetil.list");
	com.yunt.registerBonusDetil.list = {
		refresh : function(isSelect) {
			var href_string = '${path}/registerBonusDetil/list?model.guideUser.username=' + $("#model_guideUser_username").val().trim();
			if(!isSelect){
			...
			<div class="col-xs-6">
				<form class="form-inline pull-right" action="" id="search" >
					...
					<button type="button" class="btn btn-default"
						onclick="com.yunt.registerBonusDetil.list.refresh(true)">筛选</button>
				</form>
			</div>
		</div>

	</div>
	<form id="theForm" action="${path}/registerBonusDetil/list?model.guideUser.username=${model.guideUser.username }">
		...
		<div class="panel panel-default" id="all-device-detail">
			...
			<table class="table table-striped table-hover table-bordered text-center"
				...
				<tbody>
					<c:forEach items="${pageObj.results}" var="registerBonusDetil" varStatus="i">
						<!-- 系统管理员 -->
						<tr>
							<td class="text-center">${(i.index+1)+((pageObj.pageNo-1) * pageObj.pageSize)}</td>
							<td class="text-center">${registerBonusDetil.guideUser.username }</td>
							<td class="text-center">${registerBonusDetil.guideUser.name }</td>
							<td class="text-center">${registerBonusDetil.guideUser.phone }</td>
							<td class="text-center">${registerBonusDetil.sumCount }</td>
							<td class="text-center">${registerBonusDetil.sumMoney }</td>
							<td class="text-center">
								<c:if test="${fn:indexOf(user.role.operationCodes,'registerBonusDetil_view') > -1 }">
									<input type="button" value="查看" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#edit-device"
											onclick="com.yunt.index.loadInner('${path}/registerBonusDetil/view?guideUserId=${registerBonusDetil.guideUser.id }','查看推荐注册详情')" />
									&nbsp;&nbsp; 
								</c:if>
							</td>
						</tr>
