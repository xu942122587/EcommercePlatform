		...
		$(function(){
			...
			<c:forEach items="${orderGoodsList }" var="orderGoods">
				<c:if test="${'1' eq orderGoods.type}">
					new_sumMoney += ${(orderGoods.price * orderGoods.count)};
					new_sumPayMoney += ${(orderGoods.price * orderGoods.count) - orderGoods.subMoney};
					new_sumSubMoney += ${orderGoods.subMoney};
				</c:if>
			</c:forEach>
