...
<script type="text/javascript">
	Namespace.register("com.yunt.order.returnOrder.list");
	com.yunt.order.returnOrder.list = {
		refresh : function(isSelect) {
			var href_string = "${path}/order/returnList"
			...
			$.ajax({
				...
				success : function(data) {
					...
					if (data.success) {
						...
						com.yunt.order.returnOrder.list.refresh();
					} else {
...
<body>
	...
	<div class="toolbar">
		<div class="row">
			<div class="col-xs-2">
				<c:if test="${fn:indexOf(user.role.operationCodes,'order_returnInput') > -1 }">
					<c:if test="${not empty user.store.id }">
						<div class="quick-add-btn">
							<a href="javascript:void(0)"type="button" class="btn btn-danger" onclick="com.yunt.index.loadInner('${path}/order/returnInput','添加退货单');">
								<span class="glyphicon glyphicon-new-window"></span> 添加退货单
							</a>
						</div>
					</c:if>
				</c:if>
			...
			<div class="col-xs-10">
				<form class="form-inline pull-right" action="" id="search" order="form">
					<div class="form-group">
						...
						<input type="text" id="orderCode" class="form-control" value="${model.code}" placeholder="退货单号" maxlength="20">
					...
					<button type="button" class="btn btn-default" onclick="com.yunt.order.returnOrder.list.refresh(true)">筛选</button>
				</form>
			</div>
		</div>

	</div>
	<form id="theForm" action="${path}/order/returnList?model.state=${model.state}&model.code=${model.code}&model.startDate=${model.startDate}&model.endDate=${model.endDate}">
				...
				<tbody>
					<c:forEach items="${pageObj.results}" var="order" varStatus="i">
						<tr>
							...
							<td class="text-center">
								<c:if test="${fn:indexOf(user.role.operationCodes,'order_returnView') > -1 }">
									<input type="button" value="查看" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#edit-device"
											onclick="com.yunt.index.loadInner('${path}/order/returnView?id=${order.id }','查看退货单信息')" />
									&nbsp;&nbsp; 
								</c:if>
								<c:if test="${fn:indexOf(user.role.operationCodes,'order_print') > -1 }">
										<input type="button" value="补印" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#edit-device"
												onclick="window.open('${path}/order/returnPrintView?id=${order.id }','打印退货单信息')" />
										&nbsp;&nbsp; 
								</c:if>
