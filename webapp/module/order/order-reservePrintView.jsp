...
/* 			body {
		...
		<div>
			<div class="print_title_font_size"style="text-align: center;margin-bottom: 10px;font-weight: bolder;font-size: 14px;">预约商品</div>
		</div>
		...
		<table style="border:none;" class="print_title_font_size table table-striped table-hover table-bordered text-center" id="ggg" data-striped="true" data-smart-display="true">
			<thead class="text-center">
				<tr>
					...
					<th class="print_title_font_size" width="55%">名称</th>
			...
			<tbody>
				<c:forEach items="${orderGoodsList }" var="orderGoods">
					<tr >
						...
						<td class="print_title_font_size"><fmt:formatNumber value="${orderGoods.salesPrice }" pattern="#,###.##"/></td>
							...
							<tr>
								<td class="text-center" >
									<c:if test="${'3' eq orderGoods.orderSubremark.promotionLadder.promotion.type}">
										特价促销:满${orderGoods.orderSubremark.promotionLadder.ladderStart }元,返${orderSubremark.promotionLadder.promotionMoney }%
									</c:if>
									<c:if test="${'7' eq orderGoods.orderSubremark.promotionLadder.promotion.type}">
										满送促销:满${orderGoods.orderSubremark.promotionLadder.ladderStart }元,送${orderSubremark.promotionLadder.goods.name },${orderSubremark.promotionLadder.goodsNumber }个
									</c:if>
									<c:if test="${'8' eq orderGoods.orderSubremark.promotionLadder.promotion.type}">
										买赠促销:买${orderGoods.orderSubremark.promotionLadder.goodsNumberBuy }个,送${orderSubremark.promotionLadder.goods.name },${orderSubremark.promotionLadder.goodsNumber }个
									</c:if>
									<c:if test="${'9' eq orderGoods.orderSubremark.promotionLadder.promotion.type}">
										加钱换购:满${orderGoods.orderSubremark.promotionLadder.ladderStart }元,加${orderSubremark.promotionLadder.goodsPrice },换${orderSubremark.promotionLadder.goods.name }
									</c:if>
								</td>
							</tr>
						</c:if>
								...
								<tr>
									<td class="text-center" >
										<c:if test="${'3' eq orderSubremark.promotionLadder.promotion.type}">
											特价促销:满${orderSubremark.promotionLadder.ladderStart }元,返${orderSubremark.promotionLadder.promotionMoney }%
										</c:if>
		...
		<div>
			<div class="print_center print_title_font_size">定金:</div>
			<div class="print_right print_title_font_size"><span style="font-size: 22px;font-weight: bolder;"><fmt:formatNumber value="${model.onePayMoney }" pattern="#,###.#"/></span>元</div>
		</div>
		<div>
			<div class="print_center print_title_font_size">尾款:</div>
			<div class="print_right print_title_font_size"><span style="font-size: 22px;font-weight: bolder;"><fmt:formatNumber value="${model.twoPayMoney }" pattern="#,###.#"/></span>元</div>
		</div>
