...
<body> 
		...
		<div class="col-xs-12 display">
			<form onsubmit="return false;"
				...
				<div class="form-group">
					<label for="product-uuid" class="col-xs-12 control-label lantiao_lable">录入退货订单号</label>
				</div>
				<div class="col-xs-3">
					<div class="form-group">
				...
				<div class="form-group">
					<label for="product-uuid" class="col-xs-12 control-label lantiao_lable">录入退货商品</label>
				</div>
				<div class="col-xs-3">
					<div class="form-group">
						<input type="text" value="" class="form-control"
								onkeydown="if(event.keyCode==32) return false;"
								id="search_input_one" placeholder="退货商品二维码扫码框~"/>
					</div>
				</div>
				
				<div class="form-group">
					<label for="product-uuid" class="col-xs-2 control-label">退货总数量</label>
					<div class="col-xs-2" style="padding-top: 7px;"><span id="sumOneCount">0</span>件</div>
					
					<label for="product-uuid" class="col-xs-2 control-label">退货总金额</label>
					<div class="col-xs-2" style="padding-top: 7px;"><span id="sumOnePrice">0</span>元</div>
				</div>
				
				<div class="form-group">
					<label for="product-uuid" class="col-xs-12 control-label lantiao_lable">退货商品 <span style="font-size: 14px;color: red;">&nbsp;&nbsp;(退货商品将退回至原仓库!)</span></label>
				</div>
				...
				<div class="form-group col-xs-8 pull-right">
					<button type="button" class="btn btn-primary" onclick="toSave();">退货价格确认</button>
		...
		<div class="modal-dialog modal-lg" style="margin-top: 150px;">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title">退货价格确认</h4>
				</div>
				<div class="modal-body row">
					<form class="form-horizontal" id="testForm_save_order"
							role="form" onsubmit="return false;">
						<div class="form-group">
							<label for="device-description" class="col-xs-4 control-label">退货价格</label>
	...
	function toProcurement() {
		...
		$.ajax({
			url : "${path}/order/returnSaveOrUpdate",
			data : formData,
			type : "POST",
			dataType : "json",
			success : function(data) {
				if (data.success) {
					...
					setTimeout(function(){
						window.open('${path}/order/returnPrintView?id='+data.obj);
	...
	function toSave() {
		...
		if("" != sumOneCount){
			if(0 >= sumOneCount){
				toastr.error("退货商品不能为空!");
			} else{
				$("#order_goods_qrcode_id").html(sumOnePrice);
				$("#order_payMoney").val(sumOnePrice);
					...
					if (data.success) {
						if(1 == ownerType){
							...
							if(isNeedAdd){
								...
								console.log(qrcodeOne_goods_number_index)
