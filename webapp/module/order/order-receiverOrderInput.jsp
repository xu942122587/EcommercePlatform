...
<script>
	var isSuccess = true;
	Namespace.register("com.yunt.order.receiverOrder");
	com.yunt.order.receiverOrder = {
			submit : function() {
				if ($('#theForm').data('bootstrapValidator').isValid()) {
					if(0 >= $("#order_goods_qrcode_all tr:not(:hidden)").length){
						toastr.error("请先添加发货出库产品哦~");
					...
					if (isSuccess) {
						$.ajax({
							url : "${path}/order/receiverSaveOrUpdate",
							data : formData,
							type : "POST",
							async : false,
							dataType : "json",
							success : function(data) {
								if (data.success) {
									...
									window.open('${path}/order/receiverPrintView?id='+data.obj);
...
<body> 
	...
	<div class="row">
		<div class="col-xs-12">
			<div class="display">
				<form onsubmit="return com.yunt.order.receiverOrder.submit(); return false;"
					...
					<div class="form-group">
						...
						<div class="col-xs-3" style="padding-top:7px;">${model.postage }</div>
					</div>
